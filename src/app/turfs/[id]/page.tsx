    ...
    <div className="fixed inset-0 bg-[#060606] flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-5">
        ...
        <div className="mb-4">
          <div className="flex items-start justify-between mb-1">
            <h1 className="text-xl font-bold text-white flex-1 pr-4">{turf.name}</h1>
          </div>
          <div className="flex items-center gap-2 mb-2">
            <div className="flex items-center gap-1">
              ...
              <span className="text-sm font-semibold text-white">{turf.rating}</span>
              <span className="text-xs text-white/30">({turf.reviews} reviews)</span>
            </div>
        ...
        <div className="grid grid-cols-3 gap-2.5 mb-5">
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-3 text-center">
            <p className="text-lg font-bold text-white">₹{turf.price}</p>
            <p className="text-[10px] text-white/30">per hour</p>
          </div>
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-3 text-center">
            <p className="text-sm font-semibold text-white">{turf.timings.split("–")[0].trim()}</p>
            <p className="text-[10px] text-white/30">Opens</p>
          </div>
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-3 text-center">
            <p className="text-sm font-semibold text-white">{turf.distance}</p>
        ...
        {activeTab === "overview" && (
          <>
            ...
            <div className="mb-5">
              ...
              <p className="text-xs text-white/40 leading-relaxed">{turf.description}</p>
            </div>

            {/* Sizes / Court types */}
            <div className="mb-5">
              <h2 className="text-sm font-semibold text-white mb-2">Available Options</h2>
              <div className="flex flex-wrap gap-2">
                {turf.sizes.map((s) => (
                  <span key={s} className="px-3 py-1.5 bg-white/[0.04] border border-white/[0.06] rounded-lg text-xs text-white/50">{s}</span>
                ))}
              </div>
            </div>
            ...
            {/* Timings & Contact */}
            <div className="mb-6">
              <h2 className="text-sm font-semibold text-white mb-2">Info</h2>
              <div className="space-y-2.5">
                <div className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.06] rounded-xl p-3">
                  ...
                  <div>
                    <p className="text-xs text-white/60">{turf.timings}</p>
                ...
                <div className="flex items-center gap-3 bg-white/[0.03] border border-white/[0.06] rounded-xl p-3">
                  ...
                  <div>
                    <p className="text-xs text-white/60">{turf.phone}</p>
      ...
      <div className="px-5 py-4 bg-[#0a0a0a] border-t border-white/[0.06] shrink-0">
        <div className="flex items-center justify-between">
          <div>
            <span className="text-xl font-bold text-white">₹{turf.price}</span>
            <span className="text-xs text-white/30 ml-1">/hr</span>
          </div>
          <Link
            href={`/turfs/${id}/book`}
